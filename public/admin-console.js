...
function wireDelivery(){const pricing=document.getElementById('adminDeliveryPricingForm');if(pricing)pricing.onsubmit=async e=>{e.preventDefault();const out=document.getElementById('deliveryPricingResult');try{await api('/api/admin/delivery/pricing',{method:'PUT',body:JSON.stringify({route_factor:Number(document.getElementById('deliveryRouteFactor').value),active:true,vehicle_rules:[deliveryRulePayload('bike','bicycle',false,10),deliveryRulePayload('car','car',true,20),deliveryRulePayload('van','van',true,30)]})});out.innerHTML='<div class="notice">Pricing version activated.</div>';await renderActive()}catch(err){out.innerHTML='<div class="error">'+esc(err.message)+'</div>'}};document.querySelectorAll('[data-courier-decision]').forEach(button=>button.onclick=async()=>{const vehicle=prompt('Approved vehicle class (bicycle, motorbike, car or van):','')??'';if(button.dataset.courierDecision==='approved'&&!vehicle)return;try{await api('/api/admin/couriers/'+button.dataset.courierId,{method:'PATCH',body:JSON.stringify({eligibility_status:button.dataset.courierDecision,approved_vehicle_class:vehicle,approval_note:'Reviewed in Admin Delivery workspace'})});await renderActive()}catch(err){showError(err)}});document.querySelectorAll('[data-delivery-assign]').forEach(form=>form.onsubmit=async e=>{e.preventDefault();try{await api('/api/admin/deliveries/'+form.dataset.deliveryAssign+'/assign',{method:'POST',body:JSON.stringify({courier_account_id:Number(form.courier_account_id.value)})});await renderActive()}catch(err){showError(err)}})}
