...
function monetizationV2Card(model){
  ...
  const item=(role,label)=>{const x=p?.[role]||{};let value='';if(x.customer_free)value='FREE';else if(x.delivery_production_fee)value=financePct(x.owner_approved_delivery_production_rate_pct||10)+' of verified delivery price';else if(x.monthly_subscription&&x.transaction_fee)value='Subscription + transaction fee';const meta=x.delivery_production_fee?'No monthly subscription · 90-day promo first':(x.promotional_entitlement?'90-day promo before monetization':'No paid profile subscription');return '<div class="monetizationRoleRow"><div><strong>'+esc(label)+'</strong><small>'+esc(meta)+'</small></div><b>'+esc(value||'Policy not configured')+'</b></div>'};
