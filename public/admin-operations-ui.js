...
window.BusinessLifeAdminOps=Object.freeze({openSupport,openTicket,openAdmin,closeOps});
function boot(){ensureUi();document.addEventListener('abl:open-support-ticket',e=>openTicket(Number(e.detail?.ticketId)));const deepLink=Number(new URLSearchParams(location.search).get('support_ticket'));if(deepLink>0){history.replaceState({},'',location.pathname);openTicket(deepLink)}if(lazyFeatureMode)return;addButtons();document.addEventListener('abl:profile-state',()=>addButtons())}
