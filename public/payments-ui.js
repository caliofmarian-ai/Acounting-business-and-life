...
async function openPayments(){
  ...
  try{
    ...
    renderOpenOrders(orders);renderHistory(intents);if(adminAllowed){await renderAdmin();renderPayMongoSetup(document.getElementById('paymentAdmin'))}
...
function renderOpenOrders(rows){const box=document.getElementById('paymentPay');if(!rows.length){box.innerHTML='<div class="paymentEmpty">No unpaid online orders.</div>';return}const checkout=pctx.paymongo?.checkout_policy||{},enabled=Boolean(checkout.checkout_enabled),methods=(checkout.enabled_methods||[]).map(methodLabel);box.innerHTML='<div class="paymentNotice"><strong>Provider-authoritative payments</strong><p>Creating an intent never marks the order paid. Only a verified server-side provider confirmation may do that.</p>'+(methods.length?'<p><strong>Available online methods:</strong> '+esc(methods.join(' · '))+'</p>':'')+(!enabled?'<div class="paymentHold">Online checkout is HOLD on this environment. '+esc(gateBlockers(gateState(checkout.required_stage==='controlled_pilot'?'controlled_pilot':'internal_qa')).join(' '))+'</div>':'')+'</div>'+rows.map(o=>'<article class="paymentOrder"><div><small>'+esc(o.business_name)+'</small><h3>'+esc(o.order_number||('#'+o.id))+'</h3><p>'+money(o.outstanding_amount,o.currency_code||'PHP')+' outstanding • '+esc(o.payment_status)+'</p></div><button data-pay-order="'+o.id+'" type="button" '+(enabled?'':'disabled')+'>'+(enabled?'Pay securely':'Online HOLD')+'</button></article>').join('');box.querySelectorAll('[data-pay-order]:not([disabled])').forEach(b=>b.onclick=()=>createIntent(Number(b.dataset.payOrder)))}
...
function renderPayMongoSetup(box){
  ...
  const p=pctx.paymongo,w=p.webhook||{},qa=gateState('internal_qa'),live=gateState('controlled_pilot'),policy=p.checkout_policy||{};
  const qaReady=qa?.state==='READY',liveReady=live?.state==='READY';
  const card=document.createElement('div');card.className='paymentControlCard pilotReadinessCard';
  const methods=(live?.enabled_methods||qa?.enabled_methods||p.methods||[]).map(methodLabel);
  card.innerHTML='<h4>PayMongo first-pilot readiness</h4>'
    +'<p class="paymentFine">Cash remains available, but the first controlled test with real customers requires PayMongo LIVE. Sandbox is internal QA only.</p>'
    ...
    +'<div class="pilotGate '+(liveReady?'ready':'hold')+'"><span><strong>First real-customer pilot</strong><small>Requires live key + explicit live mode + signed webhook</small></span><b>'+esc(live?.state||'HOLD')+'</b></div>'
    ...
    +'<div class="providerRow"><span><strong>Customer checkout</strong><small>'+(policy.production_surface?'Production surface':'QA/preview surface')+'</small></span><b>'+(policy.checkout_enabled?'ENABLED':'HOLD')+'</b></div>'
