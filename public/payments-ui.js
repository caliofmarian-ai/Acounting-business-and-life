...
async function openPayments(){
  ...
  try{
    ...
    body.innerHTML='<div class="paymentProviderBanner '+(payReady?'ready':'notReady')+'"><div><strong>'+esc(bannerTitle)+'</strong><p>'+esc(bannerCopy)+'</p></div><span>'+(pctx.paymongo?('PAYMONGO '+String(pctx.paymongo.mode||'test').toUpperCase()):'NO PSP')+'</span></div><div data-bl-pricing="customer_checkout"></div><div class="paymentTabs"><button class="active" data-ptab="pay">Pay online</button><button data-ptab="history">My payments</button>'+(adminAllowed?'<button data-ptab="admin">Finance Admin</button>':'')+'</div><section id="paymentPay"></section><section id="paymentHistory" class="hidden"></section><section id="paymentAdmin" class="hidden"></section>';
