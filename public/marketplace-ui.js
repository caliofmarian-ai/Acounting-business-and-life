...
function reportMarketContext(context){
  if(!mtok())return mtoast('Sign in to submit a private safety report.');
  const open=window.BusinessLifeFeatureLoader?.openIncidentReport;
  if(!open)return mtoast('Safety reporting is still loading. Try again in a moment.');
  return open(context);
}
