...
function renderAccountSettings(view=accountSettingsView){
  ...
  if(view==='home'){
    ...
    workspace.innerHTML=accountSettingsHeader('Security & access','Protect the personal account used by all your profiles.')+`<div id="accountSecurityMount" class="accountSecurityStack">
      <div id="accountProtectionMount"></div>
      <div id="accountPasswordMount"></div>
      <div id="accountSensitiveActionMount"></div>
      <div id="accountSessionsMount"></div>
    </div>`;
  }
...
function openFeatureWorkspace(workspaceId){
  if(!FEATURE_WORKSPACE_IDS.includes(workspaceId)||activeSurface!=='profile'||!activeRole)return false;
  const workspace=document.getElementById(workspaceId);if(!workspace)return false;
  closeDrawer();
  document.getElementById('accountSettingsWorkspace')?.classList.add('hidden');
  hideMerchantWorkspace();
  document.getElementById('roleHub')?.classList.add('hidden');
