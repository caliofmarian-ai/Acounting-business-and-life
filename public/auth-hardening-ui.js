...
let status={google_enabled:false,email_delivery_configured:false,preview_link_enabled:false};
...
function render(mode){
  ...
  else body.innerHTML=`<form id="modernRegister" class="modernAuthForm"><label>Name<input id="regName" autocomplete="name" required></label><label>Email<input id="regEmail" type="email" autocomplete="email" required></label><label>Password<input id="regPassword" type="password" minlength="8" autocomplete="new-password" required></label><label>Phone <span>optional</span><input id="regPhone" inputmode="tel" autocomplete="tel"></label><label>Primary address <span>complete before profile activation</span><textarea id="regAddress" rows="2" autocomplete="street-address"></textarea></label><button class="modernPrimary">Create person account</button><small class="modernFine">No operational profile is activated automatically. Complete Account Settings, then onboard Customer, Merchant, Supplier, Delivery or Local Services separately.</small></form>${googleButton()}`;
