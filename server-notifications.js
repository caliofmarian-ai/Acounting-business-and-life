...
const jsonBody=express.json({limit:'30mb',verify:(req,_res,buf)=>{req.rawBody=Buffer.from(buf)}});
...
let adminReady=false;let adminAppRef=null;let shuttingDown=false;let workerTimer=null;let workerRunning=false;
...
function captureResponseChunk(chunks,chunk,encoding){
  if(chunk==null)return;
  if(Buffer.isBuffer(chunk))chunks.push(Buffer.from(chunk));
  else chunks.push(Buffer.from(String(chunk),typeof encoding==='string'?encoding:'utf8'));
}
async function forwardJson(req,res,after){
  if(!adminAppRef){
    if(!res.headersSent)res.status(503).json({error:'Admin + Support runtime is not ready'});
    return;
  }
  const chunks=[];
  ...
  res.write=function(chunk,encoding,callback){
    captureResponseChunk(chunks,chunk,encoding);
  ...
  res.once('finish',()=>{
    ...
    if(res.statusCode<200||res.statusCode>=300||!after)return;
    const text=Buffer.concat(chunks).toString('utf8');
    let data={};try{data=text?JSON.parse(text):{}}catch{}
    Promise.resolve().then(()=>after(data)).catch(e=>console.error('Post-transaction notification hook:',e.message));
  });
  try{
    adminAppRef(req,res,err=>{
      if(err&&!res.headersSent)return res.status(err.status||500).json({error:err.status?err.message:'Admin + Support request failed'});
      if(!res.headersSent&&!res.writableEnded)return res.status(404).json({error:'Route not found'});
    });
  }catch(e){
    if(!res.headersSent)res.status(502).json({error:'Admin + Support runtime unavailable'});
  }
}
...
function mountAdminRuntime(req,res,next){
  if(!adminAppRef)return res.status(503).json({error:'Admin + Support runtime is not ready'});
  return adminAppRef(req,res,next);
}
app.use(mountAdminRuntime);
...
export async function startEmbeddedNotifications(){
  if(!embeddedStartPromise){
    embeddedStartPromise=(async()=>{
      adminAppRef=await startEmbeddedAdminOperations();
      adminReady=true;
      await ensureNotificationSchema(pool);
      await bootstrapResendObservability();
      workerTimer=setInterval(runWorker,8000);
      runWorker();
      console.log('Business & Life Notifications + Admin + Support runtime ready in-process');
      return app;
    })();
  }
...
if(directExecution){
  ...
  startEmbeddedNotifications().then(()=>app.listen(port,'0.0.0.0',()=>console.log(`Business & Life notification gateway listening on ${port}`))).catch(e=>{console.error(e);process.exit(1)});
}
