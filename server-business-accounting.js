...
import { loadMerchantToday } from './merchant-today-core.js';
...
app.get('/api/merchant/today',async(req,res,next)=>{try{
  const ctx=await accountingContext(req);
  const today=await loadMerchantToday(pool,ctx);
  res.json({
    ...today,
    workspace:{
      role:ctx.role,
      active_business_id:Number(ctx.business.id),
      businesses:ctx.businesses.map(b=>({
        id:Number(b.id),name:b.name,country_code:b.country_code,currency_code:b.currency_code,
        territory_id:b.territory_id,membership_role:b.membership_role,is_primary:Boolean(b.is_primary)
      }))
    }
  });
}catch(e){next(e)}});
