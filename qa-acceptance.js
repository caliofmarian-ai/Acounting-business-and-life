...
async function runSupplierPerformanceBaseline({pool,base,secret}){
  const [admin,supplier]=await Promise.all([
    qaAccountSession({pool,base,secret,email:SUPER_ADMIN_ALIAS,role:'super_admin',label:'Supplier Performance Baseline Super Admin QA'}),
    qaAccountSession({pool,base,secret,email:SUPPLIER_ALIAS,role:'supplier',label:'Supplier Performance Baseline Supplier QA'})
  ]);
  const territoryId=await ensureQaTerritory({pool,base,adminToken:admin.token});
  await ensureSupplierApproved({pool,base,supplier,adminToken:admin.token,territoryId});

  async function timed(path,{method='GET',body}={}){
    const headers={Accept:'application/json',Authorization:'Bearer '+supplier.token};
    let payload;
    if(body!==undefined){
      headers['Content-Type']='application/json';
      payload=JSON.stringify(body);
  ...
  const switchRole=await timed('/api/me/active-role',{method:'PATCH',body:{role:'supplier'}});
  expectStatus(switchRole,200,'Supplier Performance baseline active-role switch');
  const workspaces=await timed('/api/accounting/workspaces');
  expectStatus(workspaces,200,'Supplier Performance baseline Accounting workspace');
  const hubBackgroundReady=performance.now();

  const todayStarted=performance.now();
  const [me,relationships,orders,today]=await Promise.all([
    timed('/api/supplier/me'),
    timed('/api/procurement/relationships'),
    timed('/api/procurement/orders'),
    timed('/api/supplier/v5/today')
  ]);
  for(const [label,result] of [['Supplier profile',me],['relationships',relationships],['orders',orders],['Today',today]]){
    expectStatus(result,200,'Supplier Performance baseline '+label);
  }
  const todayReady=performance.now();

  const businessId=Number(today.json?.business?.id||workspaces.json?.active_business_id||0);
  if(!businessId||Number(workspaces.json?.active_business_id)!==businessId){
    throw new Error('Supplier Performance baseline business scope mismatch.');
  }

  const logoutSupplier=await requestJson(base,'/api/auth/logout',{method:'POST',token:supplier.token,body:{}});
  expectStatus(logoutSupplier,200,'Supplier Performance baseline Supplier logout');
  const logoutAdmin=await requestJson(base,'/api/auth/logout',{method:'POST',token:admin.token,body:{}});
  expectStatus(logoutAdmin,200,'Supplier Performance baseline Admin logout');

  return{
    status:'PASS',
    wave:SUPPLIER_PERFORMANCE_BASELINE_WAVE,
    profile_switch_request_count:2,
    profile_switch_paths:['/api/me/active-role','/api/accounting/workspaces'],
    ...
    accounting_workspaces_ms:workspaces.duration_ms,
    accounting_workspaces_payload_bytes:workspaces.payload_bytes,
    hub_background_ready_ms:Number((hubBackgroundReady-switchStarted).toFixed(2)),
    supplier_me_ms:me.duration_ms,
    supplier_me_payload_bytes:me.payload_bytes,
    relationships_ms:relationships.duration_ms,
    relationships_payload_bytes:relationships.payload_bytes,
    procurement_orders_ms:orders.duration_ms,
    procurement_orders_payload_bytes:orders.payload_bytes,
    supplier_today_ms:today.duration_ms,
    supplier_today_payload_bytes:today.payload_bytes,
    today_parallel_ms:Number((todayReady-todayStarted).toFixed(2)),
    active_business_id:businessId,
    business_count:Array.isArray(workspaces.json?.businesses)?workspaces.json.businesses.length:0,
    today_counts:today.json?.counts||{},
    today_money:today.json?.money||{},
    detailed_supplier_data_loaded_on_today:true,
...
async function runSupplierPerformanceRuntimeAcceptance({pool,base,secret}){
  const [admin,supplier]=await Promise.all([
    ...
    qaAccountSession({pool,base,secret,email:SUPPLIER_ALIAS,role:'supplier',label:'Supplier Performance Runtime Supplier QA'})
  ]);
  const territoryId=await ensureQaTerritory({pool,base,adminToken:admin.token});
  await ensureSupplierApproved({pool,base,supplier,adminToken:admin.token,territoryId});
  ...
  const totalStarted=performance.now();
  const switchRole=await timed('/api/me/active-role',{method:'PATCH',body:{role:'supplier'}});
  expectStatus(switchRole,200,'Supplier Performance runtime active-role switch');
  const workspaces=await timed('/api/accounting/workspaces');
  expectStatus(workspaces,200,'Supplier Performance runtime Accounting workspace');

  const todayStarted=performance.now();
  const today=await timed('/api/supplier/v5/today');
  expectStatus(today,200,'Supplier Performance runtime Today');
  const todayReady=performance.now();

  const businessId=Number(today.json?.business?.id||workspaces.json?.active_business_id||0);
  if(!businessId||Number(workspaces.json?.active_business_id)!==businessId){
    throw new Error('Supplier Performance runtime business scope mismatch.');
  }
  if(today.json?.detail_mode!=='today'||Object.prototype.hasOwnProperty.call(today.json||{},'money_orders')){
    throw new Error('Supplier Performance runtime Today unexpectedly loaded deferred Money detail.');
  }

  const money=await timed('/api/supplier/v5/today?view=money');
  expectStatus(money,200,'Supplier Performance runtime deferred Money');
  if(money.json?.detail_mode!=='money'||!Array.isArray(money.json?.money_orders)){
    throw new Error('Supplier Performance runtime deferred Money detail is invalid.');
  }

  const orders=await timed('/api/procurement/orders');
  expectStatus(orders,200,'Supplier Performance runtime deferred full PO history');
  const catalog=await timed('/api/supplier/me');
  expectStatus(catalog,200,'Supplier Performance runtime deferred catalog/profile');

  const logoutSupplier=await requestJson(base,'/api/auth/logout',{method:'POST',token:supplier.token,body:{}});
  expectStatus(logoutSupplier,200,'Supplier Performance runtime Supplier logout');
  const logoutAdmin=await requestJson(base,'/api/auth/logout',{method:'POST',token:admin.token,body:{}});
  expectStatus(logoutAdmin,200,'Supplier Performance runtime Admin logout');

  return{
    status:'PASS',
    wave:SUPPLIER_PERFORMANCE_RUNTIME_WAVE,
    profile_switch_request_count:2,
    profile_switch_paths:['/api/me/active-role','/api/accounting/workspaces'],
    today_request_count:1,
    today_request_paths:['/api/supplier/v5/today'],
    duplicate_request_paths:[],
    profile_switch_ms:switchRole.duration_ms,
    profile_switch_payload_bytes:switchRole.payload_bytes,
    accounting_workspaces_ms:workspaces.duration_ms,
    ...
    supplier_today_ms:today.duration_ms,
    supplier_today_payload_bytes:today.payload_bytes,
    today_ready_ms:Number((todayReady-todayStarted).toFixed(2)),
    full_profile_ready_ms:Number((todayReady-totalStarted).toFixed(2)),
    active_business_id:businessId,
    business_count:Array.isArray(workspaces.json?.businesses)?workspaces.json.businesses.length:0,
    today_counts:today.json?.counts||{},
    today_money:today.json?.money||{},
    deferred_money_ms:money.duration_ms,
    deferred_money_payload_bytes:money.payload_bytes,
    deferred_orders_ms:orders.duration_ms,
    deferred_orders_payload_bytes:orders.payload_bytes,
    deferred_catalog_ms:catalog.duration_ms,
    deferred_catalog_payload_bytes:catalog.payload_bytes,
    detailed_supplier_data_loaded_on_today:false,
    full_po_history_deferred:true,
    catalog_detail_deferred:true,
    deep_finance_deferred:true,
    runtime_listener:8080,
    logout:true
  };
}
...
async function runMerchantPerformanceBaseline({pool,base,secret}){
  const [admin,merchant]=await Promise.all([
    ...
    qaAccountSession({pool,base,secret,email:MERCHANT_ALIAS,role:'merchant',label:'Merchant Performance Baseline Merchant QA'})
  ]);
  const territoryId=await ensureQaTerritory({pool,base,adminToken:admin.token});
  const workspace=await ensureMerchantApproved({pool,base,merchant,adminToken:admin.token,territoryId});
  ...
  const totalStarted=performance.now();
  const switchRole=await timed('/api/me/active-role',{method:'PATCH',body:{role:'merchant'}});
  expectStatus(switchRole,200,'Merchant Performance baseline active-role switch');
  const criticalStarted=performance.now();

  const [today,workspaces]=await Promise.all([
    timed('/api/merchant/today'),
    timed('/api/accounting/workspaces')
  ]);
  expectStatus(today,200,'Merchant Performance baseline Today');
  expectStatus(workspaces,200,'Merchant Performance baseline workspaces');
  const settledAt=performance.now();

  const businessId=Number(workspaces.json?.active_business_id||workspace.businessId);
  if(!businessId||Number(today.json?.business?.id)!==businessId){
    throw new Error('Merchant Performance baseline Today/workspace business mismatch.');
  }

  const logoutMerchant=await requestJson(base,'/api/auth/logout',{method:'POST',token:merchant.token,body:{}});
  expectStatus(logoutMerchant,200,'Merchant Performance baseline Merchant logout');
  const logoutAdmin=await requestJson(base,'/api/auth/logout',{method:'POST',token:admin.token,body:{}});
  expectStatus(logoutAdmin,200,'Merchant Performance baseline Admin logout');

  return{
    status:'PASS',
    wave:MERCHANT_PERFORMANCE_BASELINE_WAVE,
    first_open_request_count:3,
    profile_switch_request_count:1,
    critical_parallel_request_count:2,
    request_paths:[
      '/api/me/active-role',
      '/api/merchant/today',
    ...
    accounting_workspaces_ms:workspaces.duration_ms,
    accounting_workspaces_payload_bytes:workspaces.payload_bytes,
    critical_parallel_ms:Number((settledAt-criticalStarted).toFixed(2)),
    full_profile_ready_ms:Number((settledAt-totalStarted).toFixed(2)),
    active_business_id:businessId,
    business_count:Array.isArray(workspaces.json?.businesses)?workspaces.json.businesses.length:0,
    order_attention_total:Number(today.json?.orders?.attention_total||0),
...
async function runCustomerPerformanceRuntimeAcceptance({pool,base,secret}){
  const customer=await qaAccountSession({
    pool,base,secret,email:CUSTOMER_ALIAS,role:'customer',label:'Customer Performance Runtime QA'
  });
  ...
  expectStatus(switchRole,200,'Customer Performance runtime active-role switch');
  const dataStarted=performance.now();

  const [orders,deliveries,services,money]=await Promise.all([
    timed('/api/orders/mine?view=home'),
    timed('/api/delivery/mine?view=home'),
    timed('/api/services/jobs/mine?view=home'),
    timed('/api/profile-money/customer?view=home')
  ]);
  for(const [label,result] of [['orders',orders],['delivery',deliveries],['services',services],['money',money]]){
    expectStatus(result,200,'Customer Performance runtime '+label);
  }
  const settledAt=performance.now();
  ...
  if(money.json?.recent_orders!==undefined||money.json?.recent_payments!==undefined){
    throw new Error('Customer Performance runtime Money Home payload leaked deferred history.');
  }
  if((Array.isArray(deliveries.json)?deliveries.json:[]).some(x=>['delivered','failed','cancelled'].includes(x.status))){
    throw new Error('Customer Performance runtime Delivery Home payload contains closed deliveries.');
  }

  const totalPayload=orders.payload_bytes+deliveries.payload_bytes+services.payload_bytes+money.payload_bytes;

  const logout=await requestJson(base,'/api/auth/logout',{method:'POST',token:customer.token,body:{}});
  expectStatus(logout,200,'Customer Performance runtime logout');

  return{
    status:'PASS',
    wave:CUSTOMER_PERFORMANCE_RUNTIME_WAVE,
    ...
    home_payload_bytes_total:totalPayload,
    active_orders:activeOrders,
    active_deliveries:activeDeliveries,
    active_service_jobs:activeServiceJobs,
    slim_home_reads:true,
    deferred_history_excluded:true,
...
export async function runQaAcceptanceIfRequested({pool,port,env=process.env}){
  ...
  let finalResult;
  try{
    const result=config.wave===COURIER_PERFORMANCE_BASELINE_WAVE
      ...
      :config.wave===MERCHANT_PERFORMANCE_RUNTIME_WAVE
