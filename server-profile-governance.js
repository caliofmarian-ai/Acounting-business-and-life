...
async function assignedOnboardingTerritory(me,fallbackTerritoryId=null){const geo=await accountGeographySnapshot(pool,me.account.id);if(geo.assigned)return requireAssignedOpenBarangay(pool,me.account.id);if(me.account.account_mode==='company_test'&&Number.isInteger(Number(fallbackTerritoryId))&&Number(fallbackTerritoryId)>0)return{id:Number(fallbackTerritoryId),test_fallback:true};return requireAssignedOpenBarangay(pool,me.account.id)}
