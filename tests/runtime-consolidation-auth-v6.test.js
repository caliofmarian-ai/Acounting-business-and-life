...
const hardening=read('server-auth-hardening.js'),governance=read('server-profile-governance.js'),accounting=read('server-business-accounting.js'),admin=read('server-admin-operations.js'),notifications=read('server-notifications.js');
...
test('Auth Hardening exposes embedded lifecycle and remains standalone rollback-capable',()=>{
  assert.match(hardening,/export async function startEmbeddedAuthHardening\(\)/);assert.match(hardening,/export async function stopEmbeddedAuthHardening\(\)/);assert.match(hardening,/spawn\(process\.execPath, \['server-incidents\.js'\]/);assert.match(hardening,/INTERNAL_INCIDENTS_PORT \|\| 3907/);assert.match(hardening,/Business & Life auth hardening mounted in-process/);assert.match(hardening,/directExecution/);assert.match(hardening,/Business & Life auth hardening gateway listening on/);
});
...
test('parsed JSON and untouched raw streams cross the embedded hardening boundary safely',()=>{
  assert.match(hardening,/const parsedJsonBody = req\.body !== undefined/);assert.match(hardening,/Buffer\.from\(JSON\.stringify\(req\.body \?\? \{\}\)\)/);assert.match(hardening,/headers\['content-length'\] = String\(payload\.length\)/);assert.match(hardening,/delete headers\['transfer-encoding'\]/);assert.match(hardening,/if \(payload\) up\.end\(payload\); else req\.pipe\(up\)/);assert.match(notifications,/app\.post\('\/api\/notifications\/webhooks\/resend',express\.raw/);
});
test('Governance Accounting Admin and Notifications cannot bypass Auth Hardening for direct reads',()=>{
  ...
  assert.match(accounting,/upstream\(`\/api\/orders\/\$\{order\.id\}`/);assert.match(admin,/upstream\('\/api\/incidents'/);
});
