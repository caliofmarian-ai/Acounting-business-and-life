...
const hardening=read('server-auth-hardening.js'),incidents=read('server-incidents.js'),governance=read('server-profile-governance.js'),accounting=read('server-business-accounting.js'),admin=read('server-admin-operations.js'),notifications=read('server-notifications.js');
...
test('parsed JSON and untouched raw streams cross the embedded Incident boundary safely',()=>{
  assert.match(incidents,/const parsedJsonBody=req\.body!==undefined/);assert.match(incidents,/Buffer\.from\(JSON\.stringify\(req\.body\?\?\{\}\)\)/);assert.match(incidents,/headers\['content-length'\]=String\(payload\.length\)/);assert.match(incidents,/delete headers\['transfer-encoding'\]/);assert.match(incidents,/if\(payload\)up\.end\(payload\);else req\.pipe\(up\)/);assert.match(notifications,/app\.post\('\/api\/notifications\/webhooks\/resend',express\.raw/);
});
