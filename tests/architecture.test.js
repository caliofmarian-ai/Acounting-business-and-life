...
test('production entry point is the unified Notifications gateway over Admin and Support', () => {
...
test('syntax check covers the public production gateway chain', () => {
  for (const file of ['server-notifications.js','server-admin-operations.js','server-business-accounting.js','server-profile-governance.js','server-auth-hardening.js','server-incidents.js','server-delivery-finance.js','server-delivery.js','server-suppliers.js','server-services.js','server-marketplace.js','server-orders.js','server-auth.js','server-v03.js']) {
...
test('current-state document identifies the pilot roadmap and notification sequence', () => {
