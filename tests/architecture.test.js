...
test('production entry point is the profile-governance gateway with bootstrap hardening', () => {
  assert.equal(pkg.version, '0.8.7');
  assert.match(pkg.scripts.start, /--import \.\/bootstrap-env\.js/);
  assert.match(pkg.scripts.start, /server-profile-governance\.js/);
});
...
test('syntax check covers the public production gateway chain', () => {
  for (const file of ['server-profile-governance.js','server-auth-hardening.js','server-incidents.js','server-delivery-finance.js','server-delivery.js','server-suppliers.js','server-services.js','server-marketplace.js','server-orders.js','server-auth.js','server-v03.js']) {
...
test('current-state document identifies the pilot roadmap and multi-business accounting next', () => {
  const state = read('docs/CURRENT_STATE.md');
  assert.match(state, /Issue #37/);
  assert.match(state, /Issue #28/);
