...
test('public deployment evidence identifies environment and revision without exposing arbitrary values',()=>{
  ...
  assert.deepEqual(publicDeploymentEvidence({APP_ENV:'qa',RAILWAY_ENVIRONMENT_NAME:'production',RAILWAY_GIT_COMMIT_SHA:'6f0f1541ace82c8852d09a75f4f897a59d549c7e'}),{environment:'qa',revision:'6f0f1541ace8'});
...
test('repository declares one application and unambiguous public versus QA domains',()=>{
  assert.match(readme,/One application, two deployment environments/);
  assert.match(readme,/https:\/\/caliof\.com/);
  assert.match(readme,/https:\/\/preview\.caliof\.com/);
  assert.match(readme,/Internal QA only/);
});
