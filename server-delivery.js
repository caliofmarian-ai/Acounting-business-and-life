...
app.get('/api/admin/deliveries',async(req,res,next)=>{try{
  const me=await requireAdmin(req,'delivery.dispatch.manage'),territoryId=me.admin_assertion.territoryId,status=clean(req.query?.status,40);
  const values=[],where=[],bind=n=>String.fromCharCode(36)+n;
  if(territoryId!=null){values.push(territoryId);where.push('b.territory_id='+bind(values.length))}
  if(status){values.push(status);where.push('d.status='+bind(values.length))}
  const{rows}=await pool.query(`SELECT d.*,o.order_number,o.order_status,o.payment_status,b.name business_name,b.territory_id,cp.display_name courier_name FROM deliveries d JOIN orders o ON o.id=d.order_id JOIN businesses b ON b.id=d.business_id LEFT JOIN courier_profiles cp ON cp.account_id=d.courier_account_id ${where.length?'WHERE '+where.join(' AND '):''} ORDER BY d.updated_at DESC LIMIT 150`,values);
  res.json(rows.map(d=>deliveryPrivacyView(d,'admin')));
}catch(e){next(e)}})
